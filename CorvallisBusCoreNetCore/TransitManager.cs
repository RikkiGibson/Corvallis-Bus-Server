...
namespace CorvallisBusCoreNetCore
{
    using CorvallisBusCoreNetCore.DataAccess;    // Maps a 5-digit stop ID to a dictionary that maps a route number to an arrival estimate in minutes.
    using CorvallisBusCoreNetCore.Models;
    using CorvallisBusCoreNetCore.Models.Connexionz;
    using CorvallisBusCoreNetCore.WebClients;    // Exists to provide some compile-time semantics to differ between schedules and estimates.
    ...
    public static class TransitManager
    {
            ...
            Dictionary<string, List<int>> stopArrivals, DateTimeOffset currentTime)
        {
            return stopArrivals.OrderBy(stopSchedule => stopSchedule.Value.DefaultIfEmpty(int.MaxValue).Min())
                               .Select(kvp =>
                new RouteArrivalsSummary(routeName: kvp.Key, routeArrivalTimes: kvp.Value, currentTime: currentTime)).ToList();
        }
    }
}
